...
def preprocess_training_data(recently_sold_df: pd.DataFrame):
    ...
    drop_columns = [
        ...
        'storiesTotal', 'NumSimilarHomesSold', 'parking','lastSoldPrice','NearbyAvgPrice','NearbyPricePerSqFt','propertyCondition'
    ]
...
def train_and_predict(recently_sold_df: pd.DataFrame, forsale_df: pd.DataFrame, n_splits: int = 5, forsale_weight: float = 0.5) -> pd.DataFrame:
    ...
    # Split ForSale into training and test to prevent leakage
    forsale_train, forsale_test = train_test_split(forsale_df, test_size=0.3, random_state=42)

    # Preprocess the ForSale training data
    X_train_forsale = preprocess_test_data(forsale_train, preprocessor)
    y_train_forsale = forsale_train['price']
    ...
    # Combine RecentlySold and ForSale (train) data
    ...
    # Predict on held-out ForSale test set
    X_test = preprocess_test_data(forsale_test, preprocessor)
    y_pred_test = final_model.predict(X_test)

    # Add predictions and scores
    forsale_test['PredictedPrice'] = y_pred_test
    forsale_test['UndervaluedScore'] = forsale_test['PredictedPrice'] - forsale_test['price']
    ...
    plt.scatter(forsale_test['price'], forsale_test['PredictedPrice'], alpha=0.5)
    plt.plot([forsale_test['price'].min(), forsale_test['price'].max()],
             [forsale_test['price'].min(), forsale_test['price'].max()], color='red', lw=2)
    ...
    # Plot: Residuals
    residuals = forsale_test['PredictedPrice'] - forsale_test['price']
    plt.figure(figsize=(10, 6))
    plt.scatter(forsale_test['PredictedPrice'], residuals, alpha=0.5)
    plt.hlines(0, forsale_test['PredictedPrice'].min(), forsale_test['PredictedPrice'].max(), color='red')
    ...
    X_full_forsale = preprocess_test_data(forsale_df, preprocessor)
    y_full_pred = final_model.predict(X_full_forsale)

    forsale_df['PredictedPrice'] = y_full_pred
